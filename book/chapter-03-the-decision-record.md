...
This book does not propose a new platform or a new repository or a new documentation standard. It proposes four instruments, each mapped directly to one of the three architectural layers, with a fourth that prevents the layers from collapsing into each other. Each instrument survives a single test. Each one either helps someone decide or it does not belong.
...
Enterprise architecture’s job, as established previously, is to end arguments before they reach delivery. The Guardrail Canvas is the instrument through which it does that.
...
Solution architecture’s failure mode, as established, is indecision disguised as rigour. The Trade-Off Matrix is the instrument that makes that failure mode impossible to sustain.
...
Even instruments that are individually well-designed will fail if the layer structure they are meant to serve has collapsed. A Guardrail Canvas that is being used to make solution-level decisions, or a Trade-Off Matrix that is being used to declare enterprise direction, is not doing the work it was designed to do. It is doing someone else’s work, and by doing so, it is allowing the trespass identified as the root cause of architectural dysfunction.
...
When these four questions cannot be answered cleanly, the review stops. Not because the discussion is not valuable. Because a review that cannot answer these questions is not in a position to make a decision, and a review that is not in a position to make a decision is not a review —it is the beginning of the latency interval that grows when decisions are not made.
